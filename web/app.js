(function(){
  'use strict';
  const E=window.TrainerEngine, A=window.TrainerAdaptive, Cloud=window.TrainerBackend, bank=Array.isArray(window.TRAINER_DATA)?window.TRAINER_DATA:[];
  ...
  let currentView='home', tickHandle=0, toastHandle=0, lastResult=null, paletteOpen=!window.matchMedia('(max-width:900px)').matches, practicePanelOpen=true, reviewDetailOpen=true, selectedReviewId='', coachOpen=false, coachTipIndex=0, cloudStarted=false, coachReply=null;
  let cloudStatus=Cloud?.status?.()||{state:'local',label:'En este dispositivo',detail:'El progreso se guarda localmente.',ai:false,sheet:false,pending:0};
  const revealedSteps={};
  let anzan;
  ...
  function currentCoachContext(){
    const p=currentProblem(),active=state.active,a=p&&active?.answers?.[E.qid(p)];
    return {p,active,a,answered:Boolean(a),examLocked:active?.mode==='exam'};
  }
  function tipMarkup(tip){return `<article class="coach-tip"><span class="coach-tip-label">Truco curado</span><h3>${esc(tip.title)}</h3><p><strong>Cuándo:</strong> ${esc(tip.condition)}</p><p><strong>Atajo:</strong> ${esc(tip.rule)}</p><div class="coach-example">${esc(tip.example)}</div><p class="coach-trap"><strong>Control:</strong> ${esc(tip.trap)}</p></article>`;}
  function setCoachOpen(open,focus=true){coachOpen=Boolean(open);$('#coachDrawer').hidden=!coachOpen;$('#coachLauncher').setAttribute('aria-expanded',String(coachOpen));if(coachOpen){renderAecodito();if(focus)$('#closeCoach').focus();}else if(focus)$('#coachLauncher').focus();}
  function updateCloudUi(){
    const label=cloudStatus.pending?`${cloudStatus.pending} pendiente${cloudStatus.pending===1?'':'s'}`:cloudStatus.label;
    if($('#syncStatus'))$('#syncStatus').textContent=label;
    if($('#footerSyncStatus'))$('#footerSyncStatus').textContent=cloudStatus.state==='connected'?'progreso local + Supabase':'progreso protegido en este navegador';
    $('#coachDot')?.classList.toggle('online',cloudStatus.state==='connected');
    const question=$('#coachQuestion');if(coachOpen&&(!question||(!question.matches(':focus')&&!question.value)))renderAecodito();
  }
  function renderAecodito(){
    const root=$('#coachContent');if(!root||!A)return;
    const context=currentCoachContext(),rec=A.recommendation(state,bank);
    if(context.examLocked){root.innerHTML=`<div class="coach-mascot-row"><img src="${$('#mascotTip img').src}" alt="Aecodito" width="86" height="86"><div><span class="cloud-pill">Simulacro activo</span><h3>Ahora compites tú.</h3></div></div><div class="coach-lock"><strong>Aecodito espera hasta la entrega.</strong><p>No muestra pistas, trucos ni respuestas durante el examen. Marca una pregunta y vuelve en la segunda vuelta.</p></div><button class="button secondary wide" type="button" id="coachCloseLocked">Volver al examen</button>`;$('#coachCloseLocked').onclick=()=>setCoachOpen(false);return;}
    const tip=context.answered&&context.active?.mode==='practice'?A.trickForProblem(context.p,context.a,coachTipIndex):(coachTipIndex?A.catalogTip(coachTipIndex):rec.tip);
    const source=cloudStatus.ai?'IA disponible bajo demanda':'Catálogo local activo';
    const reply=coachReply?`<section class="coach-reply" aria-live="polite"><span>${coachReply.source==='openai'?'Explicación generada con IA':coachReply.source==='gemini'?'Explicación generada con Gemini':'Respuesta del catálogo verificado'}</span><h3>${esc(coachReply.reply.title)}</h3><p>${esc(coachReply.reply.message)}</p><strong>${esc(coachReply.reply.action)}</strong></section>`:'';
    root.innerHTML=`<div class="coach-mascot-row"><img src="${$('#mascotTip img').src}" alt="Aecodito, tutor de Brenda" width="86" height="86"><div><span class="cloud-pill ${cloudStatus.state==='connected'?'is-online':''}">${esc(cloudStatus.label)}</span><h3>${context.answered?'Revisemos este caso.':'¿Qué te hará más rápida hoy?'}</h3><p>${esc(source)}</p></div></div>${tipMarkup(tip)}<div class="coach-actions"><button class="button secondary small" type="button" id="coachNextTip">Otro truco</button><button class="button primary small" type="button" id="coachDrill">Entrenar recomendación</button></div><section class="coach-recommendation"><span>Ruta adaptativa</span><h3>${esc(rec.title)}</h3><p>${esc(rec.evidence)}</p><strong>${esc(rec.action)}</strong></section>${reply}<form class="coach-form" id="coachForm"><label for="coachQuestion">Pregúntale por un método rápido</label><div><input id="coachQuestion" maxlength="240" autocomplete="off" placeholder="Ej.: ¿cómo calculo porcentajes más rápido?" ${context.active&&!context.answered?'disabled':''}><button class="button primary small" type="submit" ${context.active&&!context.answered?'disabled':''}>Explicar</button></div>${context.active&&!context.answered?'<small>Responde o abre la pista para consultar este ejercicio.</small>':'<small>Aecodito usa el contexto mínimo y nunca decide tu puntaje.</small>'}</form><div class="coach-cloud-note"><strong>${esc(cloudStatus.label)}</strong><span>${esc(cloudStatus.detail)}</span>${cloudStatus.state==='local'?'<button class="text-button" id="retryCloud" type="button">Reintentar conexión</button>':''}</div>`;
    $('#coachNextTip').onclick=()=>{coachTipIndex=(coachTipIndex+1)%A.TIPS.length;coachReply=null;renderAecodito();};
    $('#coachDrill').onclick=()=>{if(state.active){notify('Termina la sesión actual antes de abrir otro entrenamiento.');return;}const unit=rec.unit;setCoachOpen(false,false);selectAndStart('practice',{count:5,mode:'adaptive',topic:unit?.area||'Todos',curriculumItem:unit?.id||'Todos',level:0});};
    $('#coachForm').onsubmit=askAecodito;
    $('#retryCloud')?.addEventListener('click',()=>startCloud(true));
  }
  async function askAecodito(event){
    event.preventDefault();const input=$('#coachQuestion'),button=$('#coachForm button'),question=input.value.trim();if(!question){input.focus();return;}
    const {p,active,a,answered}=currentCoachContext(),tip=answered?A.trickForProblem(p,a):A.catalogTip(coachTipIndex);button.disabled=true;button.textContent='Pensando…';
    const result=await Cloud.askCoach({intent:answered?'explain':'strategy',problem:p,attempt:{answered,correct:answered&&a.choice===p.ans,withinTarget:answered&&a.seconds<=(p.target||90),seconds:a?.seconds,target:p?.target,reason:a?.reason,confidence:a?.confidence},tip,userQuestion:question,sessionId:active?.id});
    coachReply={reply:result.reply,source:result.source};renderAecodito();
  }
  function startCloud(force=false){if((cloudStarted&&!force)||!Cloud)return;cloudStarted=true;Cloud.init(storage);}

  function stats(){return E.stats(state);}
  function renderHome(){
    const s=stats(), eligible=bank.filter(p=>p.practiceEligible!==false), generatedItems=eligible.filter(p=>p.origin==='generated'), total=eligible.length, generated=generatedItems.length, families=new Set(generatedItems.map(p=>p.family).filter(Boolean)).size, rec=A.recommendation(state,bank), mastered=A.unitStates(state,bank).filter(x=>x.mastered).length;
    $('#homeMetrics').innerHTML=[
      ['Banco disponible',total,`${generated} nuevos · ${families} familias nuevas`],
      ['Unidades a velocidad',mastered,`de ${syllabus.items.length} unidades del temario`],
    ...
    $('#adaptiveBrief').innerHTML=`<div><span class="adaptive-icon" aria-hidden="true">◎</span><div><p class="eyebrow">Siguiente mejor paso</p><h2>${esc(rec.title)}</h2><p>${esc(rec.evidence)}</p></div></div><div class="adaptive-brief-actions"><button class="button secondary" type="button" id="openAdaptiveCoach">Ver truco</button><button class="button primary" type="button" id="startAdaptiveBrief">Entrenar 5 casos</button></div>`;
    $('#openAdaptiveCoach').onclick=()=>setCoachOpen(true);$('#startAdaptiveBrief').onclick=()=>{if(state.active){showView(state.active.mode==='exam'?'exam':'practice');return;}selectAndStart('practice',{count:5,mode:'adaptive',topic:rec.unit?.area||'Todos',curriculumItem:rec.unit?.id||'Todos',level:0});};
    $('#dailyTitle').textContent=rec.tip.title;$('#dailyTip').textContent=`${rec.tip.condition} ${rec.tip.rule} ${rec.tip.example}`;
  ...
  function showView(name,focus=true){
    ...
    if(name==='anzan'&&coachOpen)setCoachOpen(false,false);
  ...
  function selectAndStart(kind,overrides={}){
    ...
    if(kind==='exam'&&coachOpen)setCoachOpen(false,false);
  ...
  function renderPracticeQuestion(){
    ...
    $('#nextSolutionStep')?.addEventListener('click',()=>revealPracticeStep(p,1));$('#allSolutionSteps')?.addEventListener('click',()=>revealPracticeStep(p,(p.steps||[]).length));$('#askCurrentCoach')?.addEventListener('click',()=>{coachReply=null;coachTipIndex=0;setCoachOpen(true);});bindReflection();
  }
  ...
  function finishPractice(aborted){const ids=[...(state.active?.ids||[])],variant=state.active?.variant||'variety',summary=E.finishSession(state,bank,Date.now(),aborted);if(!summary)return;summary.variant=variant;lastResult={...summary,mode:'practice',ids};save();Cloud?.queueSession(summary,ids,state,bank);renderPracticeSurface();renderHome();}
  ...
  function renderExamSurface(){const active=state.active?.mode==='exam';document.body.classList.toggle('exam-running',active);$('#examSetup').hidden=active||lastResult?.mode==='exam';$('#examSession').hidden=!active;$('#examResult').hidden=!(lastResult?.mode==='exam');if(active){renderExamQuestion();startTicker();}else if(lastResult?.mode==='exam')renderExamResult();else updateConfig('exam');}
  ...
  function finishExam(auto=false){if(!state.active||state.active.mode!=='exam')return;const dialog=$('#finishDialog');if(dialog.open)dialog.close('cancel');const ids=[...state.active.ids],summary=E.finishSession(state,bank);summary.variant='balanced';lastResult={...summary,mode:'exam',ids};save();Cloud?.queueSession(summary,ids,state,bank);renderExamSurface();renderHome();if(auto)notify('Tiempo terminado. El examen se entregó automáticamente.');}
  ...
  function bind(){
    let tipIndex=0;
    const nextTip=()=>{tipIndex=(tipIndex+1)%A.TIPS.length;const tip=A.catalogTip(tipIndex);$('#dailyTitle').textContent=tip.title;$('#dailyTip').textContent=`${tip.condition} ${tip.rule} ${tip.example}`;const mascot=$('#mascotTip');mascot.classList.remove('tip-bounce');requestAnimationFrame(()=>mascot.classList.add('tip-bounce'));};
    $('#mascotTip').addEventListener('click',nextTip);$('#nextTip').addEventListener('click',nextTip);
    $('#coachLauncher').onclick=()=>setCoachOpen(!coachOpen);$('#closeCoach').onclick=()=>setCoachOpen(false);
    Cloud?.onStatus?.(status=>{cloudStatus=status;updateCloudUi();});
    ...
    document.addEventListener('keydown',e=>{
      ...
      if(e.key==='Escape'&&coachOpen){setCoachOpen(false);return;}
  ...
  function init(){
    ...
    $('#enterApp').onclick=()=>{try{storage.setItem('trainermath_brenda_entered','1');}catch(_){}$('#accessGate').hidden=true;$('#appShell').hidden=false;startCloud();const target=state.active?(state.active.mode==='exam'?'exam':'practice'):(legacyHash?'practice':'home');showView(target,false);if(legacyHash&&!state.active){showStudy();openStudy(legacyHash[1]);}};
    if(entered){$('#accessGate').hidden=true;$('#appShell').hidden=false;startCloud();const hash=location.hash.replace('#','');const mapped={inicio:'home',home:'home',practice:'practice',exam:'exam',review:'review',anzan:'anzan'}[hash];showView(state.active?(state.active.mode==='exam'?'exam':'practice'):(legacyHash?'practice':(mapped||'home')),false);if(legacyHash&&!state.active){showStudy();openStudy(legacyHash[1]);}}
    renderHome();
  }
  init();
})();
